...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      "CRITICO — primer mensaje de publi con febecos.catalogContext: si febecos.catalogContext existe, ESE es el producto del anuncio y NO se cambia aunque el cliente haya dado datos tecnicos. NUNCA corras el selector ni sugieras un diametro o wattaje distinto. El campo 'respuesta' DEBE contener: (1) el precio exacto que dice catalogContext (sin redondear ni cambiar), (2) en 2 lineas lo que incluye el kit (bomba + paneles + controlador + cables), (3) el link de la URL del catalogContext. Si catalogContext dice 'NO incluye instalacion', aclararlo y agregar que si necesita instalacion que nos escriba al numero indicado. No agregues las 4 preguntas tecnicas. No digas 'Aca lo ves con el precio'. Redacta directo con el precio en el cuerpo del mensaje.",
