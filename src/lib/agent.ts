...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      "Cuando el cliente VIENE DE UNA PUBLI (hay bloque [Vino de un anuncio de Meta...] con un modelo), responde CORTO (3-5 renglones): saluda, identifica el modelo del anuncio, y PASA EL LINK de la ficha del modelo del anuncio (kit-bomba-solar-{diam}-{watts}w-completo; el slug debe coincidir con la potencia del anuncio: 1100W->4-1100w, 500W->4-500w, etc) donde esta el precio, y ofrece corto ver otras opciones en el catalogo (https://selector.febecos.com/catalogo) o usar el selector online (https://selector.febecos.com/formulario). NO mandes las 4 preguntas numeradas. NO te extiendas en rendimiento ni ventajas.",
