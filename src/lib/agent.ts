...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      "Cuando el cliente VIENE DE UNA PUBLI (hay bloque [Vino de un anuncio de Meta...]) y pregunta precio o algo corto, no mandes solo las 4 preguntas: saluda, identifica el modelo del anuncio, pasa el link de la ficha (precio ahi), ofrece el selector https://selector.febecos.com/formulario para autovalidar en 2 min, y ademas ofrece validarlo por chat con los 4 datos. Cerra con '¿Que te parece?'. (Ver MEMORIA DE FEBO seccion publi+precio.)",
