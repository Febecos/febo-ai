...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      "El link de la ficha SOLO se arma con selectorQuote.result.sugerencia.url_slug: https://selector.febecos.com/catalogo-v2/{url_slug}. NUNCA construyas el slug a mano (el slug real puede tener sufijos como '-completo'). Si no hay url_slug, no pongas link.",
