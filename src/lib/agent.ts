...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      `Hora actual en Argentina: ${new Date().toLocaleString("es-AR", { timeZone: "America/Argentina/Buenos_Aires", hour: "2-digit", minute: "2-digit", hour12: false })} hs. Usa el saludo correcto: 'Buen día' antes de las 12, 'Buenas tardes' de 12 a 20, 'Buenas noches' de 20 en adelante.`,
      "Cuando el cliente VIENE DE UNA PUBLI (hay bloque [Vino de un anuncio de Meta...] con un modelo), OBLIGATORIO usar DOS campos separados: campo 'respuesta' = SOLO el saludo + modelo + link ficha (2-3 lineas). Campo 'segundoMensaje' = SOLO catalogo + selector + cierre (3-4 lineas). NUNCA pongas todo en 'respuesta' con separadores como '---'. Si no usas 'segundoMensaje', estas cometiendo un error. El slug del link: kit-bomba-solar-{diam}-{watts}w-completo (1100W->4-1100w, 500W->4-500w, etc). NO digas NINGUN numero de specs. NO mandes 4 preguntas numeradas.",
