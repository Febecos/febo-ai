...
export async function runFebecosAgent(input: {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: [
      ...
      "Cuando el cliente VIENE DE UNA PUBLI (hay bloque [Vino de un anuncio de Meta...] con un modelo), SIEMPRE en la primera respuesta -sin importar que escriba: 'Precio?', 'cuanto sale', 'quiero el kit full de 500', 'me interesa', o solo el reenvio del anuncio- haces TODO esto: saluda, identifica el modelo del anuncio, PASA EL LINK de la ficha (kit-bomba-solar-{diam}-{watts}w-completo, precio ahi), ofrece el selector https://selector.febecos.com/formulario para autovalidar en 2 min, y ademas ofrece validarlo por chat con los 4 datos. El link de ficha y el del formulario son OBLIGATORIOS, no mandes solo las 4 preguntas. Cerra con '¿Que te parece?'.",
